...
Identify the current Agent's supported client ID first (for example `codex`, `claude-code`,
`cursor`, `gemini-cli`, `windsurf`, or `opencode`). Configure only that client. Omitting
`--client` may change every supported client detected on the machine and requires separate,
explicit approval.
